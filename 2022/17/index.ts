...
import { type Coordinates, north, west, render, east, serializeCoords, type Symbols } from 'utils/coordinates'
...
const shapeToSymbols = (shape: Coordinates[], symbol: string) =>
  shape.reduce<Symbols>((result, part) => {
...
const windAdjustment = (shape: Shape, step: number, winds: string[], blocks: Coordinates[]) => {
  ...
  const newShape = shape.map(s => transform(s))
