...
const move = (
  numbers: Array<{ value: number, originalIndex: number }>,
  index: number,
  log = false
) => {
  ...
  if (currentIndex === -1) throw new Error(`Number with originalIndex ${index} not found`)
  const numberToMove = numbers[currentIndex].value

  const toMove = numberToMove % (len - 1)
  const toMove2 = currentIndex + toMove
  const toMove3 =
    toMove2 < 0 // wrap from start to end
      ? len + toMove2 - 1
      : toMove2 >= len // wrap from end to start
        ? toMove2 % (len - 1)
        : toMove2
